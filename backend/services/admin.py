...
def admin_create_project(
    ...
    *,
    ...
    project = Project(
        ...
        ram_quota=ram_quota,
        ssd_quota=ssd_quota,
        status="ACTIVE",  # ИСПРАВЛЕНО: было "active"
    )
    db.add(project)
    db.commit()
    db.refresh(project)
...
def admin_change_project_status(
    ...
    # Приводим к верхнему регистру для соответствия Enum в БД
    target_status = new_status.upper() 
    
    action = "DISABLE_PROJECT" if target_status == "DISABLED" else "ACTIVATE_PROJECT"
    job = enqueue_job(db, "PROJECT", project.id, action)
    try:
        project.status = target_status  # ИСПРАВЛЕНО: будет "ACTIVE" или "DISABLED"
        db.add(project)
        db.commit()
        db.refresh(project)
        _mark_job_success(db, job)
...
def admin_list_disabled_projects(db: Session) -> List[Project]:
    return (
        db.query(Project)
        .filter(Project.status == "DISABLED")  # ИСПРАВЛЕНО: было "disabled"
        .all()
    )
