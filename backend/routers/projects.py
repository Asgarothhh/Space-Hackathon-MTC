...
def start_project_endpoint(project_id: UUID, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    require_admin(current_user)
    job = start_project(db, owner_id=current_user.id, project_id=project_id, is_admin=True)
    if job is None:
...
def stop_project_endpoint(project_id: UUID, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    require_admin(current_user)
    job = stop_project(db, owner_id=current_user.id, project_id=project_id, is_admin=True)
    if job is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Project not found or access denied")
    return {"job_id": str(job.id), "status": job.status}
...
def update_project_endpoint(project_id: UUID, payload: ProjectUpdateRequest, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    require_admin(current_user)
...
def delete_project_endpoint(project_id: UUID, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    require_admin(current_user)
    job = delete_project(db, owner_id=current_user.id, project_id=project_id, is_admin=True)
    if job is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Project not found or access denied")
    return {"job_id": str(job.id), "status": job.status}
