...
const FloatingInput = ({ label, type, value, onChange, suffix, disabled }) => {
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        disabled={disabled}
        className="w-full bg-transparent text-white text-sm outline-none pr-10 pl-4 py-4 rounded-xl disabled:opacity-50"
        style={{
...
export const LoginModal = ({ isOpen, onClose, onSwitchToRegister }) => {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      // authApi.login теперь возвращает { ...data, role }
      const result = await authApi.login(email, password);
      onClose();

      // Редирект по роли — админ идёт в /admin, остальные в /dashboard
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="px-10 pt-12 pb-10" onKeyDown={handleKeyDown}>
              ...
              <motion.button
                onClick={handleLogin}
                disabled={!canSubmit}
                className="w-full mt-6 py-3.5 rounded-xl text-white text-sm font-semibold tracking-wide transition-opacity"
                style={{
